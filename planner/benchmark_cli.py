...
def main() -> None:
    ...
    lines = ["# Sequential benchmark", "", "Synthetic deterministic benchmark. Timing fields are local runtime measurements, not warehouse or database timings.", "", f"- Truth cases: {result['truth_case_count']}", f"- Budgets: {', '.join(map(str, result['budgets']))} minutes", f"- Seeds: {', '.join(map(str, result['seeds']))}", f"- Runs: {len(result['runs'])}", "", "| Strategy | Runs | Mean resolved cases | Mean simulated minutes | False excluded cases |", "|---|---:|---:|---:|---:|"]
