  ...
  return (
    <div className="shell">
      ...
      <main id="investigation">
        ...
        <section className="review panel" id="review"><div className="panel-title"><div><p className="eyebrow">Human review boundary</p><h2>Review proposed evidence</h2></div><span className="pending">{evidenceId ? "Awaiting acceptance" : "No decision change yet"}</span></div><div className="review-grid"><div className="source-preview"><span className="doc-label">Computed candidate preview</span><div className="document"><p>{selected?.action_type.replaceAll("_", " ")}</p><strong>{selected?.target_id}</strong><dl><dt>Scope</dt><dd>{selected?.affected_shipments.join(", ")}</dd><dt>Retrieval</dt><dd>{selected?.estimated_minutes} min</dd><dt>Availability</dt><dd>{selected?.availability}</dd></dl></div><small>The structured candidate is generated from the current feasible histories. It is not accepted evidence until a reviewer verifies the named source.</small></div><div className="form"><label>Source reference<input value={source} disabled={busy || Boolean(evidenceId)} onChange={(event) => { cancelExample(); setSource(event.target.value); }} /></label><label>Proposed structured fact<textarea rows={9} value={fact} disabled={busy || Boolean(evidenceId)} onChange={(event) => { cancelExample(); setFact(event.target.value); }} spellCheck={false} /></label><label>Verified by<input value={reviewer} placeholder="Reviewer name" onChange={(event) => setReviewer(event.target.value)} /></label>{evidenceId && <label>Rejection reason<input value={rejectionReason} placeholder="Required only for rejection" onChange={(event) => setRejectionReason(event.target.value)} /></label>}{error && <p className="feedback error">{error}</p>}{message && <p className="feedback success">{message}</p>}<div className="controls"><button className="secondary" disabled={busy || Boolean(evidenceId)} onClick={propose}>Save proposal</button><button className="danger" disabled={busy || !evidenceId || !reviewer.trim() || !rejectionReason.trim()} onClick={reject}>Reject</button><button className="primary" disabled={busy || !evidenceId || !reviewer.trim()} onClick={accept}>Accept and reassess</button></div><p className="boundary">Saving a proposal never changes a shipment decision. Acceptance checks the displayed incident version and runs deterministic reassessment.</p></div></div></section>

        <section className="audit panel" id="audit"><div className="panel-title"><div><p className="eyebrow">Evidence audit</p><h2>Reviewed evidence</h2></div><span>{evidenceLog.length} records</span></div>{retractable ? <div className="retract-row"><div><strong>{retractable.evidence_id}</strong><span>{retractable.status.replaceAll("_", " ")} by {retractable.verified_by}</span></div><label>Retraction reason<input value={retractionReason} placeholder="Why is this source no longer valid?" onChange={(event) => setRetractionReason(event.target.value)} /></label><button className="danger" disabled={busy || !reviewer.trim() || !retractionReason.trim()} onClick={retract}>Retract latest reviewed evidence</button></div> : <p className="empty">No accepted or conflicting evidence can be retracted.</p>}</section>
